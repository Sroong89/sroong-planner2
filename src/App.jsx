...
export default function App() {
  ...
  // 실시간 데이터 동기화
  useEffect(() => {
    if (!syncKey) { setLoading(false); return; }
  ...
  if (loading && syncKey) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FDF8F8]">
        <Infinity size={48} className="text-[#C89B9B] animate-pulse" />
      </div>
    );
  }
        ...
        <header className="bg-white rounded-2xl p-4 shadow-sm border border-[#E8D6D6]">
          ...
          {(!syncKey || showSettings) && (
            <div className="mb-4 p-3 bg-[#FDF8F8] rounded-xl border border-dashed border-[#C89B9B] space-y-2">
              <p className="text-[10px] text-[#B48787] font-bold flex items-center gap-1">
                <Link2 size={12} /> 동기화 키 (PC와 동일하게 입력)
              </p>
              <input 
                type="text" 
                value={syncKey} 
          ...
          <div className="flex flex-col items-center gap-3">
            <div className="flex items-center justify-between w-full bg-[#FDF8F8] rounded-xl border border-[#F3E9E9] p-1 shadow-inner">
              ...
              <button onClick={() => setCurrentDate(new Date())} className="font-bold text-base">{dateKey.replace(/-/g, '. ')}.</button>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 px-2 min-w-0">
                    <input 
                      ...
                      placeholder="실제 수행한 기록" 
                      className="w-full py-1 bg-transparent outline-none text-xs text-[#8B7373] italic truncate" 
                    />
                  </div>
                </div>
              </div>
